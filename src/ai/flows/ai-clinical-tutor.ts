...
export async function explainClinicalCase(input: z.infer<typeof ExplainCaseInputSchema>) {
...
export async function analyzeTestPerformance(input: z.infer<typeof PerformanceInputSchema>) {
  const { text } = await ai.generate({
