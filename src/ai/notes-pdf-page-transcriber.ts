...
const TRANSCRIBE_PROMPT = `You are transcribing ONE page of a medical student's already-condensed revision notes (a photographed/scanned page image) into clean Markdown, for direct use as study notes in an app.
...
export async function transcribeNotesPage(imageBase64: string, mimeType: string = 'image/jpeg'): Promise<{ page?: TranscribedPage; error?: string }> {
  const MAX_ATTEMPTS = 2;
  let lastError = '';
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      const { content: raw } = await callGeminiNativeMultimodal(TRANSCRIBE_PROMPT, [imageBase64], 3000, mimeType);
