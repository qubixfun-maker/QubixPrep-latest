...
export default function ProductsPage() {
  ...
  const bundledPackIds = useMemo(() => {
    const ids = new Set<string>()
    notePacks.forEach((p: any) => {
      if (p.packType === "combo") {
        (p.includedPackIds || []).forEach((id: string) => ids.add(id))
      }
    })
    return ids
  }, [notePacks])
  ...
  const displayedPacks = useMemo(() => {
    return notePacks.filter((p: any) => p.packType === "combo" || !bundledPackIds.has(p.id))
  }, [notePacks, bundledPackIds])
        ...
        {notePacksLoading ? (
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {displayedPacks.map((pack: any) => {
              const owned = isAdmin || purchasedNotePacks.includes(pack.id)
              return (
                <Card key={pack.id} className="glass border-none overflow-hidden">
                  <CardContent className="p-6 space-y-4">
                      ...
                      {owned ? (
                        pack.packType === "combo" ? (
                          <Button
                            ...
                            onClick={() => setExpandedComboId(expandedComboId === pack.id ? null : pack.id)}
                          >
                    ...
                    {owned && pack.packType === "combo" && expandedComboId === pack.id && (
