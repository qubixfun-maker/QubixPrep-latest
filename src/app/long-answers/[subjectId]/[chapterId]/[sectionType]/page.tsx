        ...
        <div className="space-y-3">
          {items.map((item, i) => (
            <div key={i} className={`rounded-2xl glass border ${color.border} overflow-hidden transition-all duration-300`}>
              ...
              {expanded[i] && (
                <div className="px-5 pb-5 pt-1 border-t border-white/5 animate-in slide-in-from-top-2 duration-300">
                  <div className="qa-answer" dangerouslySetInnerHTML={{ __html: item.answerHtml }} />
                </div>
              )}
            </div>
          ))}
